...
The app icon should use Milo’s orange-tabby face with one recognizable village element.
...
* Safe placement within the Android adaptive-icon mask
...
The concept artwork labels the orange cat Maxine. All production assets must rename the cat Milo so Maxine remains the learner and app namesake.
...
Screen 08 of the reference file shows the finished adaptive launcher icon: Milo's orange-tabby face with a cottage-on-a-hill village element, no text, key features inside the 66dp safe circle so no launcher mask crops them. Exported PNGs (also usable as vector — recreate as `VectorDrawable` for shipping):

| File | Use |
|---|---|
| `assets/icon/app-icon-512.png` | Play Store / composed master |
| `assets/icon/ic_launcher_foreground.png` | Adaptive **foreground** layer (transparent) |
| `assets/icon/ic_launcher_background.png` | Adaptive **background** layer |
| `assets/icon/app-icon-monochrome.png` | Themed-icon / single-color mark |
| `assets/icon/app-icon-48.png` | 48px legibility check |

Build the adaptive icon with `foreground` + `background` layers in `mipmap-anydpi-v26/ic_launcher.xml`; supply the monochrome layer for Android 13+ themed icons.
